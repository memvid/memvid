...
///
/// # Arguments
/// * `memvid` - Arc-wrapped Memvid instance for thread-safe access
...
/// ```ignore
/// use std::sync::{Arc, Mutex};
/// let memvid = Arc::new(Mutex::new(Memvid::open("memory.mv2")?));
/// let handle = start_enrichment_worker(Arc::clone(&memvid), None);
///
